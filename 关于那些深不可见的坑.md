...
### 运行时异常(RuntimeException)不用抛出

- 受检查异常必须被抛出(throws)给上层调用处理，或者自己用 try/catch 处理
- 运行时异常 RuntimeException 不用声明 throws，系统会自动将它不断向上层抛出，如果外部没有处理的逻辑，则最终由 jvm 处理
- 所以处理 RuntimeException 的时候，只需要在最外层处理就行，如全局异常处理 AOP
...
- C:\Windows\System32\drivers\etc 文件夹下的 hosts 文件
- 末尾加入 `ip 域名`，如 `123.123.123.123 api.logs.yunwei`
...
- 用 maven 成功引入外部依赖，在 maven 仓库中能发现相应的 jar 包，代码也能成功编译运行，但是编写时 IDEA 却爆红“找不到该索引”，无法使用该依赖
- 此坑是因为本人为了隐藏项目中某些文件不显示，在设置中（`Settings => Editor => File Types => Ignored Files and Folders`）写入了不规范的过滤名导致。此设置将外部依赖中一些 jar 包直接过滤掉了

------

### 两个数据结构相同的类，强制转换失败

- 背景：在两个包中写了封装返回结果的类 **ResultInfo<T>**，数据结构完全相同。在写全局异常处理代码时，发生异常时会生成一个包含失败信息的 **ResultInfo<T>**，此返回结果与 controller 中引用的 **ResultInfo<T>** 不在同一个包中，因此传回给 controller 时报异常：**java.lang.ClassCastException**
- 原因：强制转换只能在有继承关系的类之间进行，两个不同包中数据结构相同的类无法进行强制转换
- 解决办法：
  1. 提取 **ResultInfo<T>** 为公共类
  2. 构造类型转换函数
...
- 项目采用了Apollo配置文件时，若没有在 pom.xml 中配置 Apollo 环境，则可以在单元测试中加上如下类似代码，实现 Apollo 环境的配置
...
- 接口参数错位问题：
  ...
  - C# 中的接口若有多个默认值的参数，则需要注意传入参数的顺序与位置。若前一个默认参数没有传递过来，可能会影响后面的默认参数。（最好与下游确认哪些参数是必须要传的，不要添加没必要的参数）

- 自己测试的时候参数乱传或漏传，可能会污染数据库，给测试部门的人造成困扰和麻烦。~~自己测试的时候尽量写对参数，或者写上明显的测试标记（如：myname_Test）~~。提测后生产环境不要乱测

- maven 加载私服失败
  
  - 具体参考 [https://www.jianshu.com/p/274c363ffd7c](https://www.jianshu.com/p/274c363ffd7c)
  - 在项目的 pom.xml 文件中加上类似如下配置
